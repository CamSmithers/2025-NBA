...
```{r}
win_three <- train(
    win ~ tmsn_avgposspts + tmsn_avgposspts_opp,
    data = usdpo,
    method = "glm",
    family = "binomial",
...
```{r}
model1_prob <- predict(win_three, win_train, type = "prob")$Win
model2_prob <- predict(win_four, win_train, type = "prob")$Win
model3_prob <- predict(win_five, win_train, type = "prob")$Win
...
plot(perf2, col = "blue", add = TRUE, lty = 2)
plot(perf3, col = "forestgreen", add = TRUE, lty = 3)
legend(0.8, 0.2, legend = c("win_model1", "win_model2", "win_model3"),
       col = c("black", "blue", "forestgreen"), lty = 2:1, cex = 0.6)
```

```{r}
vip(win_five, num_features = 20)
```
